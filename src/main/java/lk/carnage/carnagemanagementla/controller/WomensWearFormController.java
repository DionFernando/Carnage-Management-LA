...
import lk.carnage.carnagemanagementla.view.tdm.WomensTm;
...
public class WomensWearFormController implements Initializable {
    ...
    WomensBO womensBO = (WomensBO) BOFactory.getBoFactory().getBO(BOFactory.BOTypes.WOMENS);
    ...
    private void generateID() {
        try {
            String currentID = womensBO.generateWomensID();
    ...
    public void addItemOnAction(ActionEvent actionEvent){
        ...
        WomensDTO womens = new WomensDTO(id, category, Double.parseDouble(price), Integer.parseInt(qty), season);

        try {
            boolean isSaved = womensBO.addWomens(womens);
            if(isSaved) {
    ...
    private void loadAllWomens(){
        tblWomen.getItems().clear();
        try {
            ArrayList<WomensDTO> allWomens = womensBO.getAllWomens();
            for (WomensDTO womensDTO: allWomens){
                tblWomen.getItems().add(new WomensTm(womensDTO.getId(),womensDTO.getCategory(),womensDTO.getPrice(),womensDTO.getQty(),womensDTO.getSeason()));
            }
        } catch (SQLException e) {
            throw new RuntimeException(e);
        } catch (ClassNotFoundException e) {
    ...
    public void updateBtnOnAction(ActionEvent actionEvent){
        ...
        WomensDTO product = new WomensDTO(id, category, Double.parseDouble(price), Integer.parseInt(qty), season);

        try {
            boolean isUpdated = womensBO.updateWomens(product);
            if (isUpdated) {
    ...
    public void deleteBtnOnAction(ActionEvent actionEvent) {
        ...
        try {
            boolean isDeleted = womensBO.deleteWomens(id);
            if(isDeleted) {
    ...
    public void MenuBtnOnAction(ActionEvent actionEvent) throws IOException {
        Parent rootNode = FXMLLoader.load(this.getClass().getResource("/menuForm.fxml"));
